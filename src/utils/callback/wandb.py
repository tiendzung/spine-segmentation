...
class WandbCallback(Callback):
    ...
    def log_data_samples_into_tables(
        ...
        sample_image = torch.rot90(sample_image, 1, dims=(2, 3))
        sample_pred = torch.rot90(sample_pred, 1, dims=(2, 3))
        sample_label = torch.rot90(sample_label, 1, dims=(2, 3))
    ...
    def on_test_batch_end(self,
        ...
        for i in range(len(prob)):
            ...
            self.log_data_samples_into_tables(images[i], seg, labels[i], img_name, self.table)
            ...
            print(seg_out.shape) 
            affine = batch["image_meta_dict"]["original_affine"][i].numpy()

            nib.save(nib.Nifti1Image(seg_out.astype(np.uint8), affine), os.path.join(self.trainer.logger.logdir, img_name))